...
export interface Run {
  id: number; job_id: number; repo: string; issue_number: number; job_type: JobType;
  started_at: string; ended_at: string | null; result: RunResult | null;
  summary: string; next_context: string; log_path: string;
}

/** ジョブ取得と同一トランザクションで作る。 */
export function startRun(
  db: DB, v: { job_id: number; repo: string; issue_number: number; job_type: JobType; log_path: string },
): number {
  const r = db.query(`
...
export function endRun(
  db: DB, jobId: number, result: RunResult, v?: { summary?: string; next_context?: string },
): void {
  db.query(`
...
export function lastRun(db: DB, repo: string, issue: number): Run | null {
  return db.query("SELECT * FROM runs WHERE repo=? AND issue_number=? ORDER BY id DESC LIMIT 1")
