...
export const agyAdapter: AgentAdapter = {
  ...
  buildInvocation(agent: AgentConfig, o: AdapterOptions): Invocation {
    ...
    const instruction = [
      ...
      "【最重要】非対話（print）モードのため、バックグラウンドコマンドの完了待ちメッセージを出してツール呼び出しを停止してはならない（即座にセッションが中断される）。",
      "実行に時間のかかる重いコマンドの再実行は避け、コード差分・設定の精査を行い、必ず指示ファイルに指定された終了時の結果JSONファイル（*.result.json）を書き込んでから終了すること。",
    ].join("\n");
    const argv = [agent.command, "--print", instruction];
