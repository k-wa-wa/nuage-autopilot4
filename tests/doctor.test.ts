...
describe("doctor printChecks", () => {
  test("全件 ok の場合 true を返し、サマリーに正常件数が表示される", () => {
    const checks: Check[] = [
      { category: "GitHub 認証・アカウント", name: "bot account", level: "ok", detail: "@autopilot-bot" },
      { category: "実行環境・CLI ツール", name: "agent:triage", level: "ok", detail: "claude (claude)" },
  ...
  test("fatal を含む場合は false を返す", () => {
    const checks: Check[] = [
      { category: "GitHub 認証・アカウント", name: "bot account", level: "fatal", detail: "viewer 取得失敗" },
    ];
    const ok = printChecks(checks);
