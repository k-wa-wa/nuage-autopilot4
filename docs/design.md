...
| **Dashboard（超軽量コックピット）** | ローカルにキャッシュされた Issue 状態と Triage 判定結果を一覧表示し、次に何をすべきかを 3 秒で把握できる UI。 |
...
### 9. 定期巡回による改善提案
- **Story**: 自分が起票しなくても、autopilot が定期的にリポジトリを巡回してリファクタや品質向上の Issue を挙げてほしい。放っておくと劣化するコードを、思い出す手間なく継続的に良くしていくため。
- **Acceptance**: 巡回エージェントが仕様まで書いた Issue を bot として 1 件起票し、Action Required（仕様確認待ち）に並ぶこと。以降は人間が起票した Issue と同じ流れ（「OK」➔ 実装 ➔ CI ➔ 品質評価 ➔ 人間がマージ）に乗ること。未クローズの巡回 Issue がある間は次を起票せず、未消化を溜めないこと。
...
Dashboard および Chat コックピットのUIデザイン哲学、デザイントークン体系、コンポーネント設計原則、ガードレール（禁止事項）については、[ui-design.md](./ui-design.md) を参照すること。
